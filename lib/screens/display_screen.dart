...
class DisplayScreenState extends State<DisplayScreen> {
  ...
  String _userFriendlyError(Object e) {
    ...
    if (s.contains('failed to fetch') || s.contains('clientexception') || s.contains('connection')) {
  ...
  Future<bool?> _showSettingsDialog() async {
    final macController = TextEditingController(text: widget.settings.macAddress);
          ...
          actions: [
            ...
            FilledButton(
              onPressed: () {
                ...
                if (url.isNotEmpty) {
                  widget.settings.baseUrl = url.endsWith('/') ? url.substring(0, url.length - 1) : url;
                }
  ...
  Future<void> _loadFromOffline([String? fetchError]) async {
    ...
    if (cached != null && cached.isNotEmpty) {
      ...
      setState(() {
        _loading = false;
        _error = fetchError ?? 'No media found. Connect to internet to load media.';
      });
    }
  }
  ...
  Uri _streamUri(int mediaId) {
    final base = widget.settings.baseUrl.endsWith('/')
        ? widget.settings.baseUrl.substring(0, widget.settings.baseUrl.length - 1)
  ...
  void _playVideo(MediaItem media, {int retryCount = 0}) {
    ...
    if (Platform.isAndroid && retryCount == 0) {
      // Android: try streaming first with API URL; if it fails, fall back to download.
      _tryStreamThenDownload(media, videoUri);
      return;
    }
  ...
  Future<void> _downloadAndPlayVideo(MediaItem media) async {
    ...
    final seconds = (120 + (media.fileSize / (2 * 1024 * 1024)).ceil()).clamp(120, 600);
    final overallTimeout = Duration(seconds: seconds);
    try {
      await _downloadAndPlayVideoInner(media, uri).timeout(
        overallTimeout,
        onTimeout: () => throw TimeoutException('Timeout (${overallTimeout.inSeconds}s)'),
      );
    } catch (e) {
  ...
  Future<void> _downloadAndPlayVideoInner(MediaItem media, Uri uri) async {
    ...
    try {
      ...
      final ext = media.fileType.contains('mp4') ? 'mp4' : (media.fileType.contains('webm') ? 'webm' : 'mp4');
  ...
  Widget _buildPlayer() {
          ...
          child: useFile
              ? Image.file(
                  ...
                  errorBuilder: (_, __, ___) {
                    WidgetsBinding.instance.addPostFrameCallback((_) => _nextMedia());
                    return const Center(child: Text('Failed to load image', style: TextStyle(color: Colors.white70)));
                  },
                )
              : Image.network(
                  media.previewUrl,
    ...
    if (media.isVideo && _videoController != null && _videoController!.value.isInitialized) {
      return Center(
        child: AspectRatio(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text('Skipping in 3s...', style: TextStyle(color: Colors.white54, fontSize: 14)),
